...
export default function EventDashboard() {
  ...
  const meetingsCount = useMemo(
    () => events.reduce((sum, e) => sum + e.items.length, 0),
    [events]
  );

  const colorChip = (color: EventItem["color"]) => {
    if (color === "emerald")
      return "bg-emerald-50 text-teal-400 border-neutral-300";
      ...
      <div className="w-full flex justify-between items-center">
        <div className="flex flex-col justify-start items-start gap-4">
            ...
            <div className="flex justify-start items-center gap-6">
              <div className="flex justify-center items-center gap-3">
                <button
                  ...
                  className="w-8 h-8 p-2.5 rounded-lg border border-gray-100 flex justify-center items-center gap-2.5 hover:bg-gray-50"
                >
              ...
              <div className="text-neutral-600 text-lg font-semibold font-sans leading-normal">
                {monthLabel}
              </div>
            </div>
            <div className="w-24 h-7 p-1 bg-slate-200 rounded-full flex justify-center items-center">
              <div className="text-sky-900 text-xs font-medium font-sans leading-none">
          ...
          <div className="flex justify-start items-center gap-4">
            <button
              ...
              className="h-7 px-3 py-1 rounded-2xl border border-slate-200 flex justify-center items-center"
            >
              <div className="text-zinc-500 text-sm font-normal font-sans leading-tight">
                Today
              </div>
            </button>
        ...
        <div className="h-14 px-4 py-3 bg-slate-200 rounded-lg flex justify-center items-center gap-3">
          <button
            ...
            className={`w-12 h-10 px-2 py-0.5 rounded flex justify-center items-center ${
              view === "calendar"
                ? "bg-white shadow-[0px_4px_10px_0px_rgba(218,233,241,1.00)]"
                : "bg-transparent"
            }`}
          >
            <Calendar
              className={`w-6 h-6 ${
                view === "calendar" ? "text-blue-900" : "text-zinc-500"
              }`}
            />
          </button>
          <button
          ...
          >
            <List
              className={`w-6 h-6 ${
                view === "list" ? "text-blue-900" : "text-zinc-500"
              }`}
            />
          </button>
        </div>
          ...
          <>
            ...
            <div className="self-stretch px-3 grid grid-cols-7">
              {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
                <div
                  key={day}
                  className="h-14 px-4 py-3 flex justify-center items-center"
                >
                  <div className="text-zinc-500 text-xs font-medium font-sans leading-none">
                    {day}
                  </div>
                </div>
              ))}
              ...
              {cells.map((cell, idx) => {
                if (!cell.inMonth || !cell.date) {
                  return (
                    <div
                      key={idx}
                      className="h-28 p-3 bg-gray-50 rounded border border-gray-100"
                    />
                  );
                }
                ...
                return (
                  <div
                    key={idx}
                    className={`h-28 p-3 relative bg-white rounded ${
                      highlight
                  ...
                  >
                    ...
                    <div className="absolute left-[10px] top-[30px] right-2 bottom-2 flex flex-col justify-start items-start gap-1.5 overflow-y-auto pr-1 custom-scroll-thin">
                      {dayEvents?.items?.map((ev) => (
                        ...
                        >
                          <div className="flex-1 text-xs font-normal font-sans leading-none truncate">
                            {ev.title}
                          </div>
                    ...
                    {/* Day numbeer */}
                    <div
                      className={`w-5 h-5 px-2 absolute right-[5px] top-[5px] ${
                        highlight ? "bg-blue-900 text-white" : ""
