...
export default function AdminHeader({ onMenuClick }: AdminHeaderProps) {
  ...
  const userInfo = AdminData.users.find(user => user.role === "admin") || AdminData.users[5]; // Miguel Trevino
  ...
  const getUnreadCount = () => {
    return notificationData.filter((notif: any) => notif.status === "unread").length;
  };
